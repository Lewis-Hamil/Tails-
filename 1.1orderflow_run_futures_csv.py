...
# NOTE: we now import the PF version, not the old MH/Kalman one
import orderflow_tails_pf as oft
...
# ---------- CLI ----------
def parse_args():
    p = argparse.ArgumentParser(description="Run orderflow tails (PF) on OHLCV CSV")
    ...
    p.add_argument("--plots", action="store_true", help="(kept for compatibility, but PF runner has simpler plots)")
    ...
    p.add_argument("--bands-delta", default=None)
    p.add_argument("--bands-size",  default=None)
    p.add_argument("--bands-imb",   default=None)
    # PF knobs (optional)
...
def main():
    ...
    # Run PF-based model
    out_delta = oft.run_pipeline(delta, bands=bands_delta, pf_cfg=pf_cfg)
    out_size  = oft.run_pipeline(size,  bands=bands_size,  pf_cfg=pf_cfg)
    out_imb   = oft.run_pipeline(imb,   bands=bands_imb,   pf_cfg=pf_cfg)
    ...
    for label, out, series in [
        ...
        print(
            ...
            f"| Hill={_fmt(t.get('hill'))} | PE bands: P20={_fmt(bands.get('p20'))}, "
            f"P80={_fmt(bands.get('p80'))}, med={_fmt(bands.get('med'))}"
        )
